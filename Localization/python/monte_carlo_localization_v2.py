...
class ParticleFilter(object):
    ...
    def handle_observation(self, laser_scan, dt):
        ...
        for particle in self.particles:
            ...
            errors.append(self.get_prediction_error_squared(laser_scan, particle))
        ...
        maximum = np.max(np_errors)
        y = np.exp(np_errors - maximum)
        y /= y.sum()
        self.weights = y
        
        
        N_eff = 1/np.sum(np.array(self.weights)**2)
    ...
    def resample(self):
        ...
        for i in range(N):
            v = np.random.uniform()
            beta += v*2*max_w
            while beta > self.weights[idx]:
                beta -= self.weights[idx]
                idx = (idx + 1)%N
            ...
            curr_y = p.y 
            curr_theta = p.theta
            new_particles.append(Particle(i, curr_x, curr_y, curr_theta))
    ...
    def simulate_laser_scan_for_particle(self, x, y, yaw_in_map, angles, min_range, max_range):
        ...
        for angle in angles:
            abs_angle = angle + yaw_in_map
            d = min_range
            final_d = max_range
            while d < max_range:
                new_x = x + d*cos(abs_angle)
                new_y = y + d*sin(abs_angle)
                if (self.xmin < new_x < self.xmax) and (self.ymin < new_y < self.ymax):
                    occ_x, occ_y = self.metric_to_grid_coords(new_x, new_y)
                    if not self.grid_bin[occ_x, occ_y]:
                        final_d = d
                        break
                else:
                    final_d = max_range
    ...
    def get_prediction_error_squared(self, laser_scan_msg, particle):
        ...
        # print(len(angles))
        # print(len(actual_ranges))
                
        # TODO: simulate a laser scan using one of the methods of this class
        
        predicted_ranges = self.simulate_laser_scan_for_particle(particle.x, particle.y, particle.theta, angles, self.laser_min_range, self.laser_max_range)

        
        # TODO: compute the difference bwteen predicted ranges and actual ranges
        # Take the squared norm of that difference
        norm_error = np.array(predicted_ranges) - np.array(actual_ranges)
        return np.sum(norm_error**2)
