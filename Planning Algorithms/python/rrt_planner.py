...
class RRTPlanner(object):
    ...
    def sample_state(self):
        ...
        cols, rows, channels = self.world.shape
    ...
    def steer_towards(self, s_nearest, s_rand, max_radius):
        ...
        # Distance between s_nearest and s_rand
        d = s_nearest.euclidean_distance(s_rand)
        if d <= max_radius:
            x = s_rand.x
            y = s_rand.y
        else:
            # Unit vector in the direction of s_rand
            v_x = (s_rand.x - s_nearest.x)/d
            v_y = (s_rand.y - s_nearest.y)/d
            
            # Coordinates of new point along the line between s_nearest and
            # s_rand but chopped to max_radius
            x = int(s_nearest.x + v_x*max_radius)
            y = int(s_nearest.y + v_y*max_radius)
    ...
    def path_is_obstacle_free(self, s_from, s_to):
        ...
        # Distance from s_from to s_to
        d = s_from.euclidean_distance(s_to)
        
        # Making sure s_from != s_to to avoid division by 0 error
        # If the new node overlaps with a node in the tree, there are
        # no obstacles
        if d == 0:
            return True
            
        # Unit vector in the direction of s_to
        v_x = (s_to.x - s_from.x)/d
        v_y = (s_to.y - s_from.y)/d
        ...
        for i in xrange(max_checks):
            ...
            dist_frac = float(i)/max_checks * d
            inteprolated_state = State(int(s_from.x + v_x*dist_frac), int(s_from.y + v_y*dist_frac), None)
