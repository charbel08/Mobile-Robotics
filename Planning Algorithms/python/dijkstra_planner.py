...
from plotting_utils import draw_plan, draw_visited
...
class DijkstraPlanner(object):
    """
    Applies dijkstra's shortest path algorithm on a given grid world
    """
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print "Usage: dijkstra_planner.py occupancy_grid.pkl"
        sys.exit(1)
    ...
    dp = DijkstraPlanner(world)

    start_state = State(10, 10)
    dest_state = State(150, 400)
    
    plan, visited = dp.plan(start_state, dest_state)
